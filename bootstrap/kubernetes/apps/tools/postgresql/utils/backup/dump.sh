...
echo "PG_BIN: $PG_BIN"
if [ ! -d "$PG_BIN" ]; then
    echo "ERROR: PG_BIN directory does not exist: $PG_BIN"
fi
if [ ! -f "$PG_BIN/psql" ]; then
    echo "ERROR: psql not found: $PG_BIN/psql"
fi
if [ ! -f "$PG_BIN/pg_dumpall" ]; then
    echo "ERROR: pg_dumpall not found: $PG_BIN/pg_dumpall"
fi
...
function estimate_size {
    echo "Running estimate_size with query: ${ALL_DB_SIZE_QUERY}"
    result=$("$PG_BIN"/psql -tqAc "${ALL_DB_SIZE_QUERY}" 2>&1)
    echo "estimate_size result: '$result'"
    echo "$result"
}
    ...
    case $LOGICAL_BACKUP_PROVIDER in
        ...
        "s3")
            ...
            if [ -z "$size" ] || ! [[ "$size" =~ ^[0-9]+$ ]]; then
                echo "ERROR: Invalid estimated size: '$size'"
                expected_size=""
            else
                expected_size=$((size / DUMP_SIZE_COEFF))
                echo "Expected size for upload: $expected_size"
            fi
